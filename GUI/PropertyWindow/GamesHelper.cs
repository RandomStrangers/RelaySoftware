    ...
    public sealed class GamesHelper {
...
#if DEV_BUILD_NOVA
            if (!game.Running) game.Start(Player.Nova, "", int.MaxValue);
#else
...
#if DEV_BUILD_NOVA
                RoundsGameConfig.AddMap(Player.Nova, map, lvlCfg, game);
#else
        ...
        void DelMap_Click(object sender, EventArgs e) {
            try {
                ...
                LevelConfig lvlCfg = LevelInfo.GetConfig(map);
#if DEV_BUILD_NOVA
                RoundsGameConfig.RemoveMap(Player.Nova, map, lvlCfg, game);
#else
