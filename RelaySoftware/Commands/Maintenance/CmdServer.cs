...
namespace RelaySoftware.Commands.Maintenance {
    public sealed class CmdServer : Command2 {
        ...
        static void DoRestore(Player p)
        {
            if (!CheckPerms(p))
            {
                p.Message("Only Nova or the Server Owner can restore the server."); return;
            }
            Backup.Extract(p);
        }
        ...
        static void DoUpdate(Player p)
        {
            if (!CheckPerms(p))
            {
                p.Message("Only Nova or the Server Owner can update the server."); return;
            }
