...
namespace RelaySoftware.Tasks {
    public delegate void SchedulerCallback(SchedulerTask task);
    public delegate void SchedulerCallback2(SchedulerTask2 task);

    public sealed class Scheduler {

        readonly List<SchedulerTask> tasks = new List<SchedulerTask>();
        readonly List<SchedulerTask2> tasks2 = new List<SchedulerTask2>();
        readonly object taskLock2 = new object();
        volatile SchedulerTask2 curTask2; // for .ToString()
        readonly AutoResetEvent handle2 = new AutoResetEvent(false);
        ...
        public SchedulerTask2 QueueOnce2(object state, TimeSpan delay)
        {
            return EnqueueTask(new SchedulerTask2(state, delay, false));
        }
        ...
        SchedulerTask2 EnqueueTask(SchedulerTask2 task2)
        {
            lock (taskLock2)
            {
                tasks2.Add(task2);
    ...
    public class SchedulerTask2
    {
        ...
        public SchedulerTask2(object state,
                             TimeSpan delay, bool repeating)
        {
            State = state;
            Delay = delay;
